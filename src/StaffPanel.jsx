    ...
    <div className="admin-console-body">
      ...
      <main className="admin-console-main">
        ...
        {tab === 'debates' && !busy && <section className="admin-debates-log">
          <div className="admin-debate-summary"><span><i className="waiting" />{debateLog.filter((item) => item.waiting).length} waiting</span><span><i className="active" />{debateLog.filter((item) => item.active && !item.reported).length} active</span><span><i className="reported" />{debateLog.filter((item) => item.reported).length} reported</span><span><i className="completed" />{debateLog.filter((item) => !item.active && !item.waiting).length} completed</span></div>
          <div className="staff-table-wrap"><table><thead><tr><th>Status</th><th>Started</th><th>Debate</th><th>Mode</th><th>Reports</th><th>Monitor</th></tr></thead><tbody>{pageSlice(debateLog).map((debate) => <tr key={debate.roomId}><td><span className={'debate-status ' + (debate.reported ? 'reported' : debate.waiting ? 'waiting' : debate.active ? 'active' : 'completed')}>{debate.reported ? '⚑ REPORTED' : debate.waiting ? 'WAITING' : debate.active ? 'ACTIVE' : 'COMPLETED'}</span></td><td>{dateValue(debate.startedAt)}</td><td><b>{debate.statement || debate.topicId || 'Debate'}</b><small className="staff-uid">{debate.roomCode ? `Code: ${debate.roomCode} · ${debate.joinMode === 'code' ? 'Private' : 'Public'}` : debate.roomId}</small></td><td>{debate.matchMode}</td><td>{debate.reportCount || 0}</td><td>{debate.active ? <button className="admin-monitor-button" onClick={() => enterDebate(debate)}>Enter anonymously</button> : <span className="staff-not-applicable">{debate.waiting ? 'Not started' : 'Ended'}</span>}</td></tr>)}</tbody></table></div>

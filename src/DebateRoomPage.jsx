...
export default function DebateRoomPage({ debateInfo, topic, opponentName = 'Opponent', opponentRole = 'user', opponentVerified = false, isSearching = false, hostQueueCount = null, connState, connectionText, localVideoRef, remoteVideoRef, localStream, micOn, camOn, onToggleMic, onToggleCam, onReport, onLeave, onMenu, onProfile, onSignOut, messages, draft, onDraftChange, onSend, socketId, reportOpen, onCloseReport, onReportSubmitted, kickOpponent, canKick }) {
  ...
  return <div className="live-room">
    ...
    <div className="live-topic"><p><strong>TOPIC:</strong> {topic} <span>&bull;</span> <em>You:</em> <b>{side}</b></p><div className="live-topic-statuses">{hostQueueCount != null && <span className={`live-queue-count ${hostQueueCount > 0 ? 'has-waiters' : ''}`} aria-live="polite"><i aria-hidden="true" />{hostQueueCount} {hostQueueCount === 1 ? 'user' : 'users'} in queue</span>}<div className={`live-opponent-name ${isSearching ? 'live-opponent-name--searching' : ''}`} role="status" aria-live="polite" aria-label={isSearching ? 'Finding you a match' : `Debating: ${opponentName}`}>

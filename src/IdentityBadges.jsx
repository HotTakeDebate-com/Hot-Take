...
export default function IdentityBadges({ verified = false, premium = false, role = 'user', compact = false }) {
  const staffRole = ['moderator', 'admin', 'super_admin', 'owner'].includes(role) ? role : null;
  ...
  return <span className={`identity-badges${compact ? ' identity-badges--compact' : ''}`} aria-label="Account badges">
    ...
    {staffRole && <span className={`identity-badge identity-badge--staff identity-badge--${staffRole}`} title={staffRole === 'super_admin' ? 'Super Admin' : staffRole[0].toUpperCase() + staffRole.slice(1)} aria-label={staffRole === 'super_admin' ? 'Super Admin' : staffRole}>
